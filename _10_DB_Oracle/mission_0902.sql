...
/*4. 현재 점수에서 +10으로 보정한 점수를 출력하시오.
   원본 데이터에 반영되지 않습니다. */
select name,point+10 보정점수 
from users;

/*5. 1학년의 점수를 +1점씩 올려서 출력하세요. (단, 원본 데이터에 반영되지 않습니다.)*/
select name,point+1 보정점수 
from users 
where grade=1;
...
select name,point,point-10 보정점수 
from users
where point>=90;
...
select name, trunc(point,-1) 보정점수
from users;

/*18. 모든 학생의 점수를 반올림하여 출력하시오. (단, 원본 데이터에 반영되지 않습니다.)*/
select name,round(point) 보정점수
from users;
...
select concat(concat(concat(substr(jumin,1,2),'년'), concat(substr(jumin,3,2),'월')), concat(substr(jumin,5,2),'일'))
from users;
...
select name,jumin, 124-to_number(substr(jumin,1,2))+1 as 나이
from users;
